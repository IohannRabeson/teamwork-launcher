...
impl<'l, T, Message: 'l> Form<'l, T, Message>
{
    ...
    pub fn push_if(mut self, condition: bool, label: impl ToString, field_fn: impl Fn (&'l T) -> Element<'l, Message> + 'l) -> Self {
        if condition {
    ...
    pub fn view(&self, context: &'l T) -> Element<'l, Message> {
        let labels = self.fields.iter().fold(column![], |column, (label, _)|{
            column.push(text(&label))
        }).spacing(self.spacing);
        let fields = self.fields.iter().fold(column![], |column, (_, field_fn)|{
            column.push((field_fn)(context))
        }).spacing(self.spacing);
