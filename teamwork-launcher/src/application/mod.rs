...
impl iced::Application for TeamworkLauncher {
            ...
            match current {
                Screens::Main => {
                    ui::main::view(ViewContext {
                        ...
                        servers_list_view_mode: self.servers_list_view_mode,
                    })
                }
                Screens::Server(view) => {
    ...
    fn subscription(&self) -> Subscription<Self::Message> {
        ...
        Subscription::batch([
            subscription::run_with_id(self.fetch_servers_subscription_id, server_stream),
    ...
    fn process_pane_message(&mut self, message: PaneMessage) {
        match message {
            PaneMessage::Resized(pane_grid::ResizeEvent { split, ratio }) => {
                ...
                self.servers_list_view_mode = self.require_compact_mode(ratio);
            }
        }
    }
        ...
        match message {
            ...
            SettingsMessage::WindowResized { width, height } => {
                if let Some(settings) = &mut self.user_settings.window {
                    ...
                    self.servers_list_view_mode = self.require_compact_mode(self.user_settings.servers_filter_pane_ratio);
                }
            }
